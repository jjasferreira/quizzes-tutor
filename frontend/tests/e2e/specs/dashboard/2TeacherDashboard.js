describe('2022 Teacher Dashboard', () => {
  ...
  it('test if the 2022 values are correct and the 2022 and 2019 graphs are shown', () => {
    ...
    cy.get('[data-cy="quiz_stats_graph"]').eq(0).scrollIntoView().wait(3000).screenshot("testImage");
    ...
    cy.readFile(
      './tests/e2e/base-reference/baseImage.png', 'base64'
    ).then(baseImage => {
      cy.readFile(
        './tests/e2e/screenshots/dashboard/2TeacherDashboard.js/testImage.png', 'base64'
      ).then(testImage => {
				// load both pictures
